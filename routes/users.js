    ...
    User.findOne({username: user.username}, (err, userTest) =>{
        ...
        //res.json(user);*/
            ...
            if(userTest != null){
                ...
                if(isValidPassword) res.json({signUp: false, message: " Cette utilisateur est déjà inscrit." })
                else {
                    user.save( (err) => {
                        if(err) res.send({signUp: false, message: "Erreur d'insertion!"});
                        else  res.json({signUp: true, message: ""+user.fullname+" enregistrer !" })
                    })
                }
            }
            else{
                ...
                user.save( (err) => {
                    if(err) {
                        ...
                        res.send({signUp: false, message: "Erreur d'insertion!"});
                    }
                    else  res.json({signUp: true, message: ""+user.fullname+" enregistrer !" })
                })
            }  
